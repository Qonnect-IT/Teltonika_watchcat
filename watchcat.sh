...
watchcat_restart_modemmanager_iface() {
        [ "$2" -gt 0 ] && {
                /usr/bin/eventslog -i -t EVENTS -n "Watchcat" -e "Resetting current-bands to 'any' on modem: \"$1\" now."
                /usr/bin/mmcli -m any --set-current-bands=any
        }
        /usr/bin/eventslog -i -t EVENTS -n "Watchcat" -e "Reconnecting modem: \"$1\" now."
        /etc/init.d/modemmanager restart
        ifup "$1"
}

watchcat_restart_network_iface() {
        /usr/bin/eventslog -i -t EVENTS -n "Watchcat" -e "Restarting network interface: \"$1\"."
        ip link set "$1" down
        ip link set "$1" up
}

watchcat_restart_all_network() {
        /usr/bin/eventslog -i -t EVENTS -n "Watchcat" -e "Restarting networking now by running: /etc/init.d/network restart"
        ...
        while true; do
                ...
                for host in $ping_hosts; do
                        ...
                        if [ "$ping_result" -eq 0 ]; then
                                ...
                                if [ "$iface" != "" ]; then
                                        /usr/bin/eventslog -i -t EVENTS -n "Watchcat" -e "Could not reach $host via \"$iface\" for \"$((time_now - time_lastcheck_withinternet))\" seconds. Restarting \"$iface\" after reaching \"$failure_period\" seconds"
                                else
                                        /usr/bin/eventslog -i -t EVENTS -n "Watchcat" -e "Could not reach $host for \"$((time_now - time_lastcheck_withinternet))\" seconds. Restarting networking after reaching \"$failure_period\" seconds"
                                fi
                        fi
                done
        ...
        while true; do
                ...
                for host in $ping_hosts; do
                        ...
                        if [ "$ping_result" -eq 0 ]; then
                                ...
                                /usr/bin/eventslog -i -t EVENTS -n "Watchcat" -e "Could not reach $host for $((time_now - time_lastcheck_withinternet)). Rebooting after reaching $failure_period"
                        fi
                done
...
/usr/bin/eventslog -i -t EVENTS -n "Watchcat" -e "Watchcat initialized in mode ${mode}"
